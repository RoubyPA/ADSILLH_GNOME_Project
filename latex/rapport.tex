...
Nous avons donc ajouté une ligne pour définir une fonction de callback pour
...
Nous avons aussi créer la fonction correspondante dont le but et de capté tous
les événements correspondant au appuis sur les boutons de la sourie, si le
bouton appuyé et le bouton de retour, exécuter la fonction préexistante
\texttt{on\_display\_back()} et retourner \texttt{true}.
Sinon retourné \texttt{false} pour propagé l’événement et qu'il puisse être
traité comme par défaut

Nous avons encore rencontré un ou deux petit problèmes, notamment une erreur
dans la fonction que nous avions écrite, nous avions échanger le \texttt{true}
et le \texttt{false} pour le return, se qui veut dire que nous ne dissusont
l’événement
Résultat seuls notre événement marche mais les autres sont détruit et jamais
traité.
...
des manette pour Games. Les appuis sur les touches TR2 et TL2 n'étais pas
...
ont une plage de valeurs possiblement et peuvent prendre plusieurs sur un
axe unique.
%% ADD ILLUSTRATION ?

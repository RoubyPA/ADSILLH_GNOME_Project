...
Il c'est avéré que dans le problème venais du type de bouton et des evenements
lié à ces boutons. En effet contrairement à d'autre modèle les mannettes de
Xbox on des boutons qui ne sont pas simplement apuiler et relacher, mais
on une plage de valeur possible et peuveut prendre plusieur sur un axe unique.
%% ADD ILLUSTRATION ?

Nous avons posté les informations que nous avons trouvé puis nous avons
commencer a réflechire une solution de corrigé le problème. Entre temps un
autre contributeur à envoyer un patch très proche de la solution que nous
avions imaginé. Nous avons donc désidé de laisser faire le contributeur qui
fesai sa première contribution au projet.

Une parti du patch à étais acepté par le mainteneur, pour le reste il à
demandé des modiffications mais le contributeur na pas encore tous corrigé.

A l'heure ou nous ecrivons c'est quelque ligne (3 mois après la déclaration
...
vous qu'il y avais des warrnings dans la sortie de la compilation, nous
avons donc fait un patch pour les corrigés. L'utilité de se patch est limité
car les warnings que nous avons corrigé ne resqué pratiquement pas de cosé un
crash, mais sa permet d'avoir une compilation plus propre.
...
dû communiquer de manière directe avec la communauté GNOME par IRC.  À
ce jour, nous avons envoyé trois patch, dont un a été accepté.
...
a été difficile de concilier le fait d'avoir à rendre un projet dans
presque chacunes des UE lors du second semestre. Cela s'est vu
amplifié par le fait que nous avons tous des niveaus techniques assez
disparates (cette remarque s'étend à la promotion), et que ce qui peut
être fait en deux heures pour l'un peut nécessiter deux jours pour
l'autre.

De plus, n'étants pas développeurs, ou ayant une expérience limitée
sur des projets conséquents, l'activité de contribution nous demande
d'accorder du temps pour déchiffrer le code, pour compiler
correctement le projet, pour chercher des bugs à notre portée, etc.

Ces deux points expliquent pourquoi nous avons peu avancés sur le
projet lors du second semestre.

D'une manière plus générale, cette expérience fut intéressant car elle

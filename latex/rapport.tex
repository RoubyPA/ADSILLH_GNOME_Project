...
par example nous pouvons créer des "Pull Requests" comme sur github
...
\subsubsection{Bug pulseaudio}
Un bug avec pulseaudio a étais declaré pour Games, en effet le programme crashé
quand le server pulseaudio est éteint et qu'on tente de lancer un jeux. Un
message d'erreur est retrouner dans le terminal si le programme est lancer a
partir d'un terminal.
\begin{verbatim}
Assertion 'p' failed at pulse/simple.c:273, function pa_simple_write(). 
Aborting.
\end{verbatim}
Il a donc fallu reproduire le bug, pour cela il a fallu personnaliser notre
configuration de pulseaudio pour qui ne se relance pas automatiquement.
\begin{verbatim}
/etc/pulse/client.conf:
   autospawn = no
\end{verbatim}
Nous avons aussi utiliser un commande pour simuler le crash du server pulseaudio.
\begin{verbatim}
$ pulseaudio --kill
\end{verbatim}
A partir de là nous avons tracer le bug dans le code source. Le bug vener d'une
fonction de la bibliotheque pusleaudio qui resevais une structure ``null'' en
entré et passé par un assert qui met donc fin au programme avec le message
...
\newline
Finalement le mainteneur a opter pour la première solution et à corriger le bug.

\subsection{Nos patchs}
% TODO PA: Cette partie a besion d'une relecture
\subsubsection{Mise à jour du fichier HACKING}
Le premier patch (\href{http://bugzilla.gnome.org/show_bug.cgi?id=788692}{ici}) 
que nous avons envoyés, est en reponse à un bug report, il ajoute une 
...
\subsubsection{Retour à la collection avec les touches Alt+Left}

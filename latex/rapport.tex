...
%% Gautier : je pense qu'on devrait enlever cette partie, elle m'a l'air superflue
...
des personnes qui sont le plus experimentées pour aider les autres 
...
fonction de la bibliothèque pusleaudio qui reservait une structure ``null'' en
entrée et passait par un assert qui met donc fin au programme avec le message
évoqué précédemment. Nous avons donc commencé à réflechir à une solution.
La solution la plus simple était de contourner la fonction qui provoque le crash
avec un test de la valeur de la structure et un ``return'' prématuré dans la fonction
appelante si la valeur est ``null'' pour bypasser le crash. \newline
Nous avons aussi pensé à afficher un message à l'utilisateur et lui demander
s'il voulait lancer quand même le jeu mais sans le son. \newline
La première solution nous a paru trop simple et pas suffisament fiable, et la
seconde un peu trop compliqué à mettre en oeuvre sur un projet que l'on découvrait juste.
Nous avons donc décidé de poster sur le rapport de bug notre analyse du problème.
\newline
Finalement le mainteneur a opté pour la première solution et a
corrigé le bug.
